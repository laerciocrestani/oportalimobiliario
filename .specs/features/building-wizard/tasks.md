...
- [ ] **T-07** POST/PATCH building com endereço; `GET /builder/cep/{cep}` (Http::fake ViaCEP).
- [ ] **T-08** `PUT .../structure` (torres + andares) só rascunho; 409 se publicado.
- [ ] **T-09** `PUT .../unit-grid` (planta típica, exceção redesenha andar, códigos `101…`); 422 andar com 0 unidades.
- [ ] **T-10** Defaults + pivôs de adicionais; DTO une adicionais do prédio + unidade (unidade não remove o do prédio).
- [ ] **T-11** `POST .../generate-description` (provider env, fake HTTP); PATCH `published` bloqueia sem preço.
- [ ] **T-12** OpenAPI completo dos endpoints novos/alterados.

## Frontend — construtora

- [ ] **T-13** Rotas wizard + persistência por step + “Continuar cadastro” na listagem.
- [ ] **T-14** Step 1 (CEP + endereço) + Vitest.
- [ ] **T-15** Step 2 + `BuildingMassing` CSS clicável + Vitest de interação.
- [ ] **T-16** Step 3 (grade, exceção, ficha, herança, adicionais).
- [ ] **T-17** Step 4 (mídia interna/externa, descritivo, IA, switch rascunho).
