...
export function ProposalsPage() {
  ...
  const canManageProposals = can('proposals.manage')
  ...
  async function load() {
    try {
      ...
      const [nextTemplates, nextVariables] = await Promise.all([
        builderApi.listProposalTemplates(),
        builderApi.listProposalVariables(),
      ])
      setTemplates(nextTemplates)
      setVariables(nextVariables)
    } catch {
      setError('Não foi possível carregar os modelos de proposta.')
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    if (!permissionsLoading && canManageProposals) {
      void load()
    }
  }, [canManageProposals, permissionsLoading])
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      if (editingId === null) {
        await builderApi.createProposalTemplate(payload)
      } else {
        await builderApi.updateProposalTemplate(editingId, payload)
      }
  ...
  function startEdit(template: ProposalTemplate) {
  ...
  async function handleDelete(template: ProposalTemplate) {
    setError(null)

    try {
      await builderApi.deleteProposalTemplate(template.id)
  ...
  if (!permissionsLoading && !canManageProposals) {
    return (
      <BuilderDashboardShell title="Propostas">
        <p className="text-sm text-muted-foreground">Você não tem permissão para gerenciar propostas.</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Modelos do tenant. Inativos não aparecem na emissão da proposta.</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
        ...
        <Card>
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={(event) => void handleSubmit(event)}>
              ...
              <div className="space-y-2">
                <Label htmlFor="proposal-body">Texto (Markdown)</Label>
                <Textarea
                  id="proposal-body"
                  ref={bodyRef}
                  rows={12}
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="proposal-active">Modelo ativo (aparece na emissão)</Label>
              </div>
